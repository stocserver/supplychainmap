            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7">
                {filtered.map((industry) => (
                    ...
                    >
                        ...
                        <Link href={`/industries/${industry.slug}${country !== 'US' ? `?region=${country}` : ''}`} aria-label={`Open ${industry.name}`} className="absolute inset-0 hidden md:block" />
                    </div>
                ))}
            </div>
                ...
                >
                    <div className="mb-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            <Link
                                href={`/industries/${hovered.slug}?view=visual${country !== 'US' ? `&region=${country}` : ''}`}
                            ...
                            <Link
                                href={`/industries/${hovered.slug}?view=value-chain${country !== 'US' ? `&region=${country}` : ''}`}
                                className="flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-200 hover:text-slate-900 transition-colors"
                            >
                            ...
                            <DialogHeader>
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex gap-2">
                                        <Link
                                            href={`/industries/${mobilePreview.slug}?view=visual${country !== 'US' ? `&region=${country}` : ''}`}
                                        ...
                                        <Link
                                            href={`/industries/${mobilePreview.slug}?view=value-chain${country !== 'US' ? `&region=${country}` : ''}`}
                                            className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-slate-100 py-2 text-sm font-medium text-slate-700 hover:bg-slate-200 active:bg-slate-300 transition-colors"
                                            onClick={() => setMobileOpen(false)}
                                        >
