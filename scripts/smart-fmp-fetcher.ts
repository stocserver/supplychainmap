...
async function fetchCompanyData(ticker: string) {
    ...
    // Priority: quote.marketCap > profile.mktCap/marketCap > keyMetrics.marketCapTTM
    const marketCap = quote?.marketCap || profile?.mktCap || profile?.marketCap || keyMetrics?.marketCapTTM || 0
