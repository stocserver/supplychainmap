// Sequential importer (one ticker at a time) using FMP only
...
async function upsertCompany(ticker: string) {
  ...
  const latestIncome: any = (data.incomeStatements as any[])?.[0]
  const latestBalance: any = (data.balanceSheets as any[])?.[0]
  const latestCashFlow: any = (data.cashFlowStatements as any[])?.[0]
...
async function main() {
  ...
  const limit = limitFlag ? parseInt(limitFlag.split('=')[1], 10) : 1
  const delayFlag = process.argv.find(a => a.startsWith('--delay='))
  const delayMs = delayFlag ? parseInt(delayFlag.split('=')[1], 10) : 1500

  console.log(`🔎 Loading tickers from Supabase (limit: ${limit})`)
